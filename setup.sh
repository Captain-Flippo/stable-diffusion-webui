...
cd ..
mv stable-diffusion-2-1-base/v2-1_512-ema-pruned.ckpt stable-diffusion-webui-on-aws/models/Stable-diffusion/
...
cp v2-inference.yaml stable-diffusion-webui-on-aws/models/Stable-diffusion/v2-1_512-ema-pruned.yaml

# change ownership of the web UI so that a regular user can start the server
sudo chown -R ubuntu:ubuntu stable-diffusion-webui-on-aws/

# start the server as user 'ubuntu'
sudo -u ubuntu nohup bash stable-diffusion-webui-on-aws/webui.sh --listen > log.txt